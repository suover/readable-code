...
public class OutputHandler {
	...
	public void showPassListForSelection(List<StudyCafePass> passes) {
		...
		for (int index = 0; index < passes.size(); index++) {
			...
			System.out.printf("%d. %s%n", index + 1, pass.display());
		}
	}
	...
	public void askLockerPass(StudyCafeLockerPass lockerPass) {
		...
		String askMessage = String.format(
			"사물함을 이용하시겠습니까? (%s)", lockerPass.display()
		);
	...
	/**
	 * 최종 주문 결과 출력 (이용권 + 사물함 + 할인금액 + 총 결제금액)
	 */
	public void showOrderResult(StudyCafeOrder order) {
		System.out.println();
		System.out.println("이용 내역");
		System.out.println("이용권: " + order.getSelectedPass().display());
		if (order.getLockerPass() != null) {
			System.out.println("사물함: " + order.getLockerPass().display());
		}

		// 이벤트 할인
		int discountPrice = order.getDiscountAmount();
		if (discountPrice > 0) {
